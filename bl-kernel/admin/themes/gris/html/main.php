...
<div id="toolbar" class="d-flex p-1">
	...
	<div id="media-button" class="editor-button pr-2">Media</div>
	<div id="delete-button" class="editor-button pr-2">Delete</div>
</div>
<script>
var _messageTimeout = 5; // Second for hide the message
// Show a text in the message topbar
function showMessage(text, cssClass='') {
	$("#message").html(text).removeClass("message-warning").addClass(cssClass).show();
	setTimeout(function(){
		$("#message").html("");
...
function showAlert(text) {
	showMessage(text, "message-warning");
}
</script>
...
// Append text in the editor at the mouse position
function editorAddContent(text) {
	const pos = _editor.codemirror.getCursor();
	_editor.codemirror.replaceRange(text, pos);
}
...
$(document).ready(function() {
	...
	$(document).on("click", "#view-button", function() {
		if (_page.type=="draft") {
			showAlert("You need publish before render this page");
		} else {
	...
	$(document).on("click", "#media-button", function() {
		// Get all files related to the current page
		Ajax.getFiles(_page.key).then(function(files) {
